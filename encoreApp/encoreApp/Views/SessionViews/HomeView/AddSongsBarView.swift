...
struct AddSongsBarView: View {
    ...
    var body: some View {
        VStack {
        if self.musicController.showNetworkAlert {
            Text("No Internet connection")
                ...
                .onAppear{
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                        self.musicController.showNetworkAlert = false
                    }
                }
        }
        ...
        Button(action: {
            if !(self.musicController.appRemote?.isConnected ?? false) {
                ...
                if !(self.musicController.appRemote?.isConnected ?? false) {
                    ...
                    self.musicController.playerPlay()
                    self.musicController.appRemote?.connect()
                }
            }
            if !self.musicController.isPaused {
            ...
            ZStack {
                ...
                Image(systemName: (self.musicController.isPaused) ? "play.circle.fill" : "pause.circle.fill")
                    .font(.system(size: 35, weight: .light))
                    .foregroundColor(self.colorScheme == .dark ? Color.white : Color.black)
            }
        }//.disabled(!musicController.buttonReady)
    }
    
    var skipButton: some View {
        Button(action: { self.musicController.playerSkipNext() }) {
